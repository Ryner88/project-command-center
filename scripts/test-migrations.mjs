...
try {
  ...
  for (const name of [
    "20260414220000_init",
    "20260420120000_add_proposal_domains",
    "20260420133000_add_proposal_start_date"
  ]) {
  ...
  run(["migrate", "status"]);
  assert.equal(
    (await db.proposal.findUniqueOrThrow({ where: { id: proposalData.id } })).proposalSeedId,
    seed.id
  );
  assert.equal(
    (await db.export.findUniqueOrThrow({ where: { proposalId: proposalData.id } })).id,
    "upgrade_export"
  );
  const other = await db.user.create({ data: { id: "other_user", email: "other@example.test" } });
  await assert.rejects(
    db.proposal.create({ data: { ...proposalData, id: "bad_owner", userId: other.id } })
  );
  await assert.rejects(
    db.export.create({
      data: {
...
`
  ...
  await Promise.all([
    db.user.count(),
    db.briefingItem.count(),
    db.proposalSeed.count(),
    db.proposal.count(),
    db.export.count()
  ]);
