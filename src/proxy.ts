...
function secure(response: NextResponse) {
  ...
  response.headers.set(
    ...
    `default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; img-src 'self' data:; object-src 'none'; script-src 'self' 'unsafe-inline'${developmentEval}; style-src 'self' 'unsafe-inline'; connect-src 'self'`
  );
...
function reject(request: NextRequest, status: number, message: string) {
  if (request.nextUrl.pathname.startsWith("/api/"))
    return secure(NextResponse.json({ error: message }, { status }));
...
export async function proxy(request: NextRequest) {
  ...
  if (!SAFE_METHODS.has(request.method)) {
    ...
    if (!origin || origin !== request.nextUrl.origin)
      return reject(request, 403, "This request did not come from this application.");
  }
  ...
  if (!(await verifySessionValue(request.cookies.get(SESSION_COOKIE)?.value)))
    return reject(request, 401, "Sign in to continue.");
  return next();
}
