...
describe("operation monitoring", () => {
  beforeEach(() => { global.reliabilityState = undefined; vi.restoreAllMocks(); });
  ...
  it("returns correlation IDs and records successful work", async () => {
    ...
    const response = await route(new NextRequest("http://localhost/api/test", { headers: { "x-request-id": "request-123" } }));
  ...
  it("returns a safe error and records the failure without its message", async () => {
    ...
    const route = observedRoute("test.failure", async () => { throw new Error("client secret value"); });
    const response = await route(new NextRequest("http://localhost/api/test"));
    expect(response.status).toBe(500);
    await expect(response.json()).resolves.toMatchObject({ error: "The service could not complete this request." });
