...
declare global {
  var reliabilityState: ReliabilityState | undefined;
}
...
export function observedRoute<TArgs extends unknown[]>(
  ...
  return async (request: NextRequest, ...args: TArgs) => {
    ...
    try {
      ...
      write(response.status >= 500 ? "error" : "info", {
      ...
      const response = NextResponse.json(
        { error: "The service could not complete this request.", requestId: context.requestId },
        { status: 500 }
      );
      response.headers.set("x-request-id", context.requestId);
      response.headers.set("x-operation-id", context.operationId);
