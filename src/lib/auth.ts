...
function accessPassword() { return process.env.APP_ACCESS_PASSWORD ?? ""; }
function sessionSecret() { return process.env.SESSION_SECRET ?? accessPassword(); }
...
export function isAccessConfigured() { return Boolean(accessPassword() && sessionSecret()); }
export function isAuthenticationRequired() { return process.env.NODE_ENV === "production" || isAccessConfigured(); }
...
export const sessionCookieOptions = {
  ...
  secure: process.env.NODE_ENV === "production", path: "/", maxAge: SESSION_LIFETIME_SECONDS
};
export async function getCurrentUserId() { return "demo-user"; }
