...
export const PATCH = observedRoute<[C]>("projects.update", async (r: NextRequest, c: C) => {
  try {
    const { id } = await c.params;
    const p = await updateProject(id, projectUpdateSchema.parse(await r.json()));
    if (!p) return NextResponse.json({ error: "Project was not found." }, { status: 404 });
    ...
    const p = getErrorPayload(e, "Project could not be updated.");
