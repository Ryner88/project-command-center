...
export const PATCH = observedRoute<[C]>("tasks.update", async (r: NextRequest, { params }: C) => {
  try {
    const { id } = await params;
    const t = await updateTask(id, taskUpdateSchema.parse(await r.json()));
    if (!t) return NextResponse.json({ error: "Task was not found." }, { status: 404 });
    ...
    const p = getErrorPayload(e, "Task could not be updated.");
