...
export const GET = observedRoute<[C]>("proposals.versions", async (_: NextRequest, c: C) => {
  const { id } = await c.params;
  return NextResponse.json({ data: await listProposalVersions(id) });
});
export const PATCH = observedRoute<[C]>("proposals.edit", async (r: NextRequest, c: C) => {
  try {
    const { id } = await c.params;
    const p = await editProposal(id, proposalEditSchema.parse(await r.json()));
    if (!p) return NextResponse.json({ error: "Proposal was not found." }, { status: 404 });
    ...
    const p = getErrorPayload(e, "Proposal could not be saved.");
