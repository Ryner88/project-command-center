...
export const PATCH = observedRoute<[StatusRouteProps]>("proposals.status", async (request: NextRequest, { params }: StatusRouteProps) => {
  try {
    ...
    await recordOwnerMutation({ action: "proposal.status_changed", entityType: "proposal", entityId: id, requestId: request.headers.get("x-request-id"), metadata: { status: input.status } });
