...
export const users = {
  ...
  ensure: (id: string, email: string, name: string) =>
    prisma.user.upsert({
      where: { email },
      update: { name },
...
export const seeds = {
  list: (userId: string) =>
    prisma.proposalSeed.findMany({
      where: { userId, archivedAt: null },
      orderBy: { createdAt: "desc" }
    }),
  first: (userId: string) =>
    prisma.proposalSeed.findFirst({
      where: { userId, archivedAt: null },
      orderBy: { createdAt: "asc" }
    }),
  get: (userId: string, id: string, db: Client = prisma) =>
    db.proposalSeed.findFirst({
      where: { id, userId, archivedAt: null }
    }),
  bySource: (
    userId: string,
    sourceType: ProposalSeedInput["sourceType"],
    sourceReference: string
  ) => prisma.proposalSeed.findFirst({ where: { userId, sourceType, sourceReference } }),
  create: (userId: string, input: ProposalSeedInput, db: Client = prisma) =>
    db.proposalSeed.create({
      data: {
        userId,
...
export const proposals = {
  ...
  edit: (userId: string, id: string, data: Prisma.ProposalUpdateInput) =>
    transaction(async (tx) => {
      const current = await tx.proposal.findFirst({ where: { id, userId } });
      if (!current) return null;
      const last = await tx.proposalVersion.aggregate({
        where: { proposalId: id },
        _max: { version: true }
      });
      await tx.proposalVersion.create({
        data: {
          ...
          snapshot: current as unknown as Prisma.InputJsonObject
        }
      });
  ...
  versions: (userId: string, id: string) =>
    prisma.proposalVersion.findMany({
      where: { userId, proposalId: id },
      orderBy: { version: "desc" }
    })
};

export const exports = {
  list: (userId: string, proposalId: string) =>
  ...
  upsert: (
    userId: string,
    data: { proposalId: string; fileName: string; filePath: string; mimeType: string }
  ) =>
    transaction(async (tx) => {
...
export const projects = {
  ...
  create: (userId: string, data: Prisma.ProjectUncheckedCreateWithoutUserInput) =>
    prisma.project.create({ data: { ...data, userId } }),
  update: (userId: string, id: string, data: Prisma.ProjectUpdateManyMutationInput) =>
    prisma.project.updateManyAndReturn({ where: { userId, id }, data })
};
...
export const tasks = {
  create: async (
    userId: string,
    projectId: string,
    data: Omit<Prisma.TaskUncheckedCreateInput, "userId" | "projectId">
  ) =>
    transaction(async (tx) => {
      if (!(await tx.project.findFirst({ where: { id: projectId, userId, archivedAt: null } })))
        throw new AppError(404, "Project was not found.");
      return tx.task.create({ data: { ...data, userId, projectId } });
    }),
  update: (userId: string, id: string, data: Prisma.TaskUpdateManyMutationInput) =>
    prisma.task.updateManyAndReturn({ where: { userId, id }, data })
};
